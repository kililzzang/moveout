...
export async function buildArchiveHtml({ title, bodyText, media, defectSummaryLines }) {
  ...
  for (const m of media) {
    ...
    if (isVideo(m)) {
      // 사진과 달리 동영상은 base64로 박아 넣으면 파일이 지나치게 커져서,
      // 원본 저장소 URL을 그대로 참조하는 실제 재생 가능한 <video>로 넣는다
      // (저장소가 나중에 사라지면 이 부분만 재생이 끊길 수 있다는 뜻).
      parts.push(`<video src="${m.url}" controls style="max-width:100%;"></video>`);
      continue;
    }
