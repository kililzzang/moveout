...
// fetchRepairHistoryForUnitKey와 같은 용도의 청소 버전 — 하자보수/청소 클립보드
// 둘 다 같은 unitKey(직전 점검의 inspections.id)로 조회한다.
export async function fetchCleaningHistoryForUnitKey(supabase, unitKey) {
  if (!unitKey) return null;
  const { data, error } = await supabase
    .from('work_orders')
    .select('cleaning_status, cleaning_items, cleaning_completed_at')
    .eq('unit_key', unitKey)
    .order('created_at', { ascending: false })
    .limit(1)
    .maybeSingle();
