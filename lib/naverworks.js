...
// 로그인한 사람의 access_token으로 게시판에 글을 쓴다. body는 이미 조립된 텍스트를
// 그대로 받는다(제목/본문 조립은 웹앱A의 buildNaverWorksTitle/buildNaverWorksBody
// 담당 — 여기서는 API 호출만 한다).
//
// ⚠️ 확인 필요: 사진을 본문 중간에 "인라인"으로 끼워넣는 게 이 API로 되는지 확인된
// 바 없다(웹앱A와 논의한 내용 — naverworks-post-format.md 참고). 이 함수는 그 자리에
// 사진 URL을 텍스트 링크로 풀어 쓰는 안전한 방식만 시도한다 — 화면(브라우저 자동화)
// 만큼 예쁘게는 안 나올 수 있지만, 최소한 사진이 통째로 누락되는 일은 없다.
export async function postToBoard({ accessToken, boardId, title, body, imageUrls }) {
  const lines = [body];
  if (imageUrls && imageUrls.length) {
    lines.push('', '--- 사진/동영상 ---', ...imageUrls);
  }
