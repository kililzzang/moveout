...
// <script>만 금지) — 그래서 순수 텍스트 링크 대신 <img> 태그로 바꿔서 실제로
// 사진은 인라인으로 뜨는 것까지 확인됨. 동영상은 <video> 태그도 같이 시도하되
// (게시판 뷰어가 이 태그를 안 그려줄 수도 있어서) 안전하게 링크도 같이 남긴다 —
// <video>가 안 뜨면 최소한 링크로는 볼 수 있게.
  ...
  const mediaHtml = (media || [])
    .map((m) => {
      ...
      if (isVideo) {
        return (
          caption +
          `<video src="${m.url}" controls style="max-width:100%;"></video><br>` +
          `<div><a href="${m.url}">${escapeHtml(m.url)}</a></div>`
        );
      }
