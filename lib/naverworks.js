...
export async function postToBoard({ accessToken, boardId, title, body, media }) {
  ...
  const mediaHtml = (media || [])
    .map((m) => {
      ...
      // 요청(박길일님) — 표시 크기만 480px로 캡. 실제 파일 용량·화질은 그대로라
      // 클릭하면 원본 화질로 볼 수 있다(썸네일을 따로 만드는 방식은 아님).
      return caption + `<img src="${m.url}" style="max-width:480px;width:100%;height:auto;" />`;
    })
    .join('<br>');
