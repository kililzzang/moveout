...
export async function postToBoard({ accessToken, boardId, title, body, media }) {
  ...
  const mediaHtml = (media || [])
    .map((m) => {
      ...
      // 2026-09-15: 사진이 max-width:100%라 화면 폭 꽉 채워서 스크롤이 길어진다는
      // 요청(박길일님) — 표시 크기만 480px로 캡. 처음엔 "클릭하면 원본 화질로 볼 수
      // 있다"고 가정했는데, 실제로는 네이버웍스 뷰어가 축소된 <img>를 탭해도 확대를
      // 안 해줬다(박길일님 확인) — 그래서 <a>로 감싸서 탭하면 무조건 원본 이미지
      // URL이 새 창/탭으로 열리게 만들었다(원본 화질 그대로 확인 가능).
      return caption + `<a href="${m.url}" target="_blank" rel="noopener"><img src="${m.url}" style="max-width:480px;width:100%;height:auto;" /></a>`;
    })
    .join('<br>');
