...
// 묶음과 하자 사진 묶음이 시작되는 지점에 한 번씩 큰 글씨로 구분선을 넣는다. 크기는
// 본문(BODY_FONT_PX)의 3배로 요청받았다. lib/postMedia.js가 매긴 group('table'·
// 'general'·'defect')이 바뀌는 순간만 감지해서 넣으므로, 사진이 몇 장이든 묶음당
// 한 번만 나온다.
const GROUP_DIVIDER_LABEL = { general: '내부사진', defect: '하자사진' };
const GROUP_DIVIDER_FONT_PX = BODY_FONT_PX * 3;
  ...
  const mediaHtml = (media || [])
    .map((m, idx) => {
      ...
      if (GROUP_DIVIDER_LABEL[m.group] && m.group !== prevGroup) {
        divider = `<div style="font-size:${GROUP_DIVIDER_FONT_PX}px;">-------------------------${GROUP_DIVIDER_LABEL[m.group]}</div>`;
      }
      prevGroup = m.group;
    ...
    .join('<br>');
  const rawBody = mediaHtml ? `${bodyHtml}<br><br>${mediaHtml}` : bodyHtml;
