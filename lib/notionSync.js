...
// 2026-09-17 신설 — 노션팀 요청: 노션 페이지 클릭 시 진행상황이 한눈에 보이게
// 본문 맨 위에 체크리스트 형태 진행 현황을 자동으로 만들어 붙인다.
function buildProgressTracker(wo) {
  const repairDone = wo.repair_status === 'completed';
  const repairNA = wo.repair_status === 'not_applicable';
  const cleaningDone = wo.cleaning_status === 'completed';
  const billingDone = wo.overall_status === 'completed';
  const inspectionDone = !!wo.inspection_completed_at || wo.inspection_status === 'completed';
  const repairLine = repairNA
    ? '- [x] 보수완료 — 해당없음'
    : `- [${repairDone ? 'x' : ' '}] 보수완료${repairDone ? '' : ' — ' + (REPAIR_STATUS_MAP[wo.repair_status] || '대기중')}`;
  return [
    '**📍 진행 현황**', '- [x] 접수',
    `- [${inspectionDone ? 'x' : ' '}] 점검완료${wo.inspector_email ? ` (${wo.inspector_email})` : ''}`,
    repairLine,
    `- [${cleaningDone ? 'x' : ' '}] 청소완료${cleaningDone ? '' : ' — ' + (CLEANING_STATUS_MAP[wo.cleaning_status] || '대기중')}`,
    `- [${billingDone ? 'x' : ' '}] 청구완료`,
  ].join('\n');
}
...
export async function syncWorkOrderToNotion(wo) {
  ...
  // 2026-09-17: 노션팀 요청 — wo.title이 없으면 unit_key(해시값)가 그대로 페이지
  // 제목으로 떠서 알아보기 어려웠다. building/room이 있으면 "건물명 호실번호"로
  // 사람이 읽을 수 있는 제목을 만든다.
  const displayTitle = wo.title || (wo.building && wo.room ? `${wo.building} ${wo.room}호` : null) || wo.unit_key || 'work order';
  const properties = {
    이름: { title: [{ text: { content: displayTitle } }] },
