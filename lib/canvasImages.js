...
export function buildChecklistImageV2(state, SECTIONS) {
  ...
  ctx.beginPath(); ctx.moveTo(pad, y); ctx.lineTo(W - pad, y); ctx.stroke();
  y += 26;
  // 총 예상 비용 금액도 V1과 마찬가지로 빨간색으로 통일(박길일님 요청).
  ctx.font = 'bold 16px sans-serif';
  const countTxt = '하자 ' + badRows.length + '건 · 총 예상 비용 ';
  const totalTxt = fmtWon(totalAmt) + '원';
  ctx.fillStyle = '#2A5C8A';
  ctx.fillText(countTxt, pad, y);
  ctx.fillStyle = '#B23A26';
  ctx.fillText(totalTxt, pad + ctx.measureText(countTxt).width, y);
