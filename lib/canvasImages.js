...
export function buildChecklistImageV1(state, SECTIONS) {
  ...
  y += 22;
  let totalAmt = 0;
  rows.forEach((row) => { totalAmt += parseInt(row.entry.amount, 10) || 0; });
  ctx.font = 'bold 14px sans-serif';
  ctx.fillStyle = '#2A5C8A';
  ctx.fillText('총 예상 비용: ' + fmtWon(totalAmt) + '원', pad, y);

  return dataURLtoFile(canvas.toDataURL('image/png'), '00_호실점검_체크리스트.png');
}
...
export function buildChecklistImageV2(state, SECTIONS) {
  ...
  return dataURLtoFile(canvas.toDataURL('image/png'), '00_하자요약표.png');
}
