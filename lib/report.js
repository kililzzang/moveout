...
export function buildAttachmentPlan(state) {
  ...
  function handle(label, entry) {
    ...
    const defectPhotos = (entry.photos || []).filter((p) => p.isDefect);
    (entry.photos || []).filter((p) => !p.isDefect).forEach((p) => {
      general.push({ url: p.url, contentType: p.contentType, label });
    });
    // 2026-09-17: 항목 상태가 'bad'가 아니어도(정상/해당없음 항목에서 사진 한 장만
    // 나중에 "하자"로 재분류한 경우) 그 사진은 defects 쪽에 실려야 한다 — 원래는
    // entry.status==='bad'일 때만 defects를 만들어서, 그런 사진이 general에서도
    // 빠지고(isDefect 필터) defects에도 안 실려(status 게이트) 통째로 report에서
    // 사라지는 문제가 있었다(사진 정상/하자 재분류 기능 추가하며 발견).
    if (entry.status === 'bad' || defectPhotos.length > 0) {
      n++;
