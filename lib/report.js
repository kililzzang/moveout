...
// 폐기물 처리 / 스티커·부착물 제거는 "청소" 비용, 그 외 하자는 "유지보수" 비용으로 분류.
export function isCleaningCategory(label) {
  return /폐기물|스티커|부착물/.test(label || '');
}
...
export function buildOfficialFormReport(state, latestEntry) {
  ...
  const totalCleaning = cleaningFee + cleaningCategoryTotal;
  lines.push('청소 ' + (totalCleaning ? fmtWon(totalCleaning) : 'X'));
  lines.push('유지보수 ' + fmtWon(maintenanceTotal - cleaningCategoryTotal));
  lines.push('');
...
function buildCurrentDamageBlock(state) {
  ...
  return {
    text: lines.join('\n'),
    cleaningTotal: cleaningFee + cleaningCategoryTotal,
    maintenanceTotal,
  };
}
