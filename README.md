...
- **로그인(네이버웍스 OAuth)** — 처음엔 "Supabase Auth(이메일 로그인)"으로 적어뒀었는데,
  ...
  - ✅ 실제 로그인 끝까지 테스트 완료(박길일님, 팀장님 계정 둘 다 성공). 처음엔 리다이렉트
    주소가 배포마다 바뀌는 임시 주소(VERCEL_URL)로 계산돼서 "유효하지 않은 클라이언트 정보"
    오류가 났었는데, `NEXT_PUBLIC_BASE_URL`을 고정 배포주소로 명시하고 해결됨.
- **네이버웍스 자동게시(API 직접 호출)** — 로그인 붙였으니 이제 브라우저 자동화 없이 서버가
  직접 게시할 수 있다.
  - `oauth_tokens` 표 — 로그인할 때 받은 access_token/refresh_token 저장(로그인 콜백에서 자동
    저장), 게시 시점에 만료됐으면 자동 갱신
  - `app/api/naverworks/post` (POST, body: `{postId}`) — post_queue에서 그 행을 가져와
    **지금 로그인한 사람의 계정으로** 게시. 성공하면 `post_queue.status`를 `posted`로 갱신
  - ⚠️ **아직 실제 게시는 한 번도 안 해봄** — `NAVERWORKS_BOARD_ID` 환경변수가 없어서 못 돌려봄
    (아래 "필요한 것" 참고). 그리고 사진을 본문 중간에 "인라인"으로 못 넣을 수도 있어서(API가
    지원하는지 미확인), 일단 본문 뒤에 사진 링크를 순서대로 풀어 쓰는 방식으로 만들어뒀다 —
    브라우저 자동화만큼 안 예쁠 수 있음.
  - ChecklistApp.jsx에 이 라우트를 호출하는 버튼은 아직 없음(웹앱A 쪽에서 "게시" 버튼을
    이 라우트로 연결해주면 브라우저 자동화를 완전히 대체할 수 있음)

### 지금 필요한 것
- **`NAVERWORKS_BOARD_ID`** — 실제 점검 리포트가 올라갈 게시판 ID. 지금 브라우저 자동화로
  올리고 있는 그 게시판의 ID를 확인해서 Vercel 환경변수에 추가해야 자동게시 테스트 가능

## 아직 안 된 것 (다음 단계)
1. **체크리스트 화면 자체 이식** — 웹앱A 쪽에서 진행 중(ChecklistApp.jsx에 폼 UI·사진 업로드·
   표준가 학습·리포트 조립·post_queue 적재까지 이미 상당 부분 구현됨).
2. ~~로그인 실사용 테스트~~ — 완료(위 참고).
3. **역할별 화면 분리** — 지금은 로그인 여부만 확인하고 역할(admin/inspector/cleaner/repair)은
   세션에 담아만 두고 화면 분기는 아직 안 함.
4. **네이버웍스 자동게시 실사용 테스트** — 코드는 만들었지만 `NAVERWORKS_BOARD_ID`가 없어서
   실제 게시는 아직 한 번도 안 해봄. 게시판 ID 채우고 "게시" 버튼을
   `/api/naverworks/post`로 연결하는 작업 남음(위 참고).
